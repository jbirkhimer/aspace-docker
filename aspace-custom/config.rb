...
# gyb of Smithsonian set the db_url
#AppConfig[:db_url] = "jdbc:mysql:loadbalance://lassb-service01.si.edu:3306,lassb-service02.si.edu:3306,lassb-service03.si.edu:3306/archivesspace?user=<secret>&password=<secret>&useUnicode=true&characterEncoding=UTF-8"
# AppConfig[:db_url] = "jdbc:mysql://lassb-service01.si.edu,lassb-service02.si.edu,lassb-service03.si.edu/archivesspace?user=<secret>&password=<secret>&useUnicode=true&characterEncoding=UTF-8&failOverReadOnly=false"
#
...
## gyb of Smithsonian.  Keep this at 8080 because now apache listens on 443 and routes to 8080
...
# gyb of Smithsonian trying to change it to AND; this was the default but it didn't work in 2.40
AppConfig[:solr_params] = { "q.op" => "AND" }
#
#
...
# gyb of Smithsonian added the plugins we use
# # mds of Quotient added refid_gen_service on 2017-01-27
AppConfig[:plugins] = ['local',  'lcnaf', 'archivesspace-delete-safety-plugin', 'refid_gen_service', 'printcss', 'drmtest', 'asck', 'si-ead-importer', 'savingmodal', 'aspace-import-excel', 'si-ead-exporter', 'editable_external_ids', 'subject_term_type']
# #AppConfig[:plugins] = ['local',  'lcnaf', 'archivesspace-delete-safety-plugin', 'aspace-search-identifier', 'refid_gen_service', 'printcss', 'drmtest', 'asck', 'si-ead-importer', 'savingmodal']
#
#
...
# gyb of Smithsonian changed the admin pw
AppConfig[:default_admin_password] = "<secret>"
#
#
...
# gyb of Smithsonian changed the page_size values
...
# gyb of Smithsonian changed the thread_count and records_per_thread # commenting this out for testing 10/16/18
# #AppConfig[:indexer_records_per_thread] = 500
...
# gyb of Smithsonian turned off PUI_indexer
...
# gyb of Smithsonian (actually Quotient), but Chintan says this is not necessary
# AppConfig[:frontend_proxy_url] = "https://lassb-vmaspace.si.edu/"
...
# gyb of Smithsonian changed from 1 hour to 4 hours
AppConfig[:session_expire_after_seconds] = 14400
#
...
# gyb of Smithsonian added for LDAP
# us.sinet.si.edu
# AppConfig[:authentication_sources] =[{
#         :model => 'LDAPAuth',
#         :hostname => 'si-usdc02.us.sinet.si.edu',
#         :port => 636,
#         :base_dn => 'dc=US,dc=SINET,dc=SI,dc=EDU',
#         :username_attribute => 'sAMAccountName',
#         :attribute_map => {:cn => :name},
#         :bind_dn => 'CN=SirisMCI,ou=Service Accounts,ou=Enterprise,dc=US,dc=SINET,dc=SI,dc=EDU',
#         :bind_password => '<secret>',
#         :encryption => :simple_tls
# },{
#         :model => 'LDAPAuth',
#         :hostname => 'si-usdc01.us.sinet.si.edu',
#         :port => 636,
#         :base_dn => 'dc=US,dc=SINET,dc=SI,dc=EDU',
#         :username_attribute => 'sAMAccountName',
#         :attribute_map => {:cn => :name},
...
# },{
#         :model => 'LDAPAuth',
#         :hostname => 'si-usdc03.us.sinet.si.edu',
#         :port => 636,
#         :base_dn => 'dc=US,dc=SINET,dc=SI,dc=EDU',
#         :username_attribute => 'sAMAccountName',
#         :attribute_map => {:cn => :name},
...
AppConfig[:report_pdf_font_paths] = proc { ["#{AppConfig[:backend_url]}/reports/static/fonts/arial/ARIAL.TTF"] }
...
# gyb of Smithsonian changed show_external_ids
...
# gyb of Smithsonian changed the identifier_delimiter from blank to dot.
AppConfig[:record_inheritance][:archival_object][:composite_identifiers] = {
  :identifier_delimiter => '.'
}
##}
